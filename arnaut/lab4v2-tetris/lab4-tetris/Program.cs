...
void LockBlock()
{
    ...
    if (CheckCollideOnShapeSwap(holdId))
    {
        isGameRunning = false;
    }
...
void HandleInput()
{
    switch (input.Key)
    {
        ...
        case ConsoleKey.S:
            ...
            {
                // holdPos += new Vector2(0, 1);
                new MoveDown().HandleInput(ref holdPos);
                timer = 0;
            }
            break;
        ...
        case ConsoleKey.A:
            if (!CheckCollideOnMove(new Vector2(-1, 0)))
                new MoveLeft().HandleInput(ref holdPos);
        ...
        case ConsoleKey.D:
            if (!CheckCollideOnMove(new Vector2(1, 0)))
                new MoveRight().HandleInput(ref holdPos);
                // holdPos += new Vector2(1, 0);
            break;
    }
